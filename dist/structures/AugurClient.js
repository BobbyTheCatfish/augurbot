...
class AugurClient extends discord_js_1.Client {
  ...
  start() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (this.config.events.includes("messageCreate")) {
        this.on("messageCreate", (message) => __awaiter(this, void 0, void 0, function* () {
          ...
          if (this.moduleManager.events.has("message")) {
            ...
            if (newMsg) {message = newMsg;} else {return;}
            let i = 0;
      ...
      if (this.config.events.includes("messageUpdate")) {
        this.on("messageUpdate", (old, message) => __awaiter(this, void 0, void 0, function* () {
          ...
          if (isEdited && this.moduleManager.events.has("messageEdit")) {
            ...
            if (newMessage) {message = newMessage;} else {return;}
            let i = 0;
      ...
      if (this.config.events.includes("messageReactionAdd")) {
        this.on("messageReactionAdd", (reaction, user) => __awaiter(this, void 0, void 0, function* () {
          ...
          if (this.moduleManager.events.has("messageReactionAdd")) {
            ...
            if (newReact) {reaction = newReact;} else {return;}
            const newMessage = yield this.fetchMsg(reaction.message);
            if (newMessage) {reaction.message = newMessage;} else {return;}
            ...
            while (i < events.length) {
              const [file, handler] = events[i];
              try {
                if (yield handler(reaction, user)) {break;}
                i++;
              } catch (error) {
      ...
      const events = (((_a = this.config) === null || _a === void 0 ? void 0 : _a.events) || [])
                .filter(event => ![
                  "message",
                  "messageUpdate",
                  "interactionCreate",
      ...
      for (const event of events) {
        this.on(event, (...args) => __awaiter(this, void 0, void 0, function* () {
          ...
          if (this.moduleManager.events.has(event)) {
            ...
            let i = 0;
            const events2 = Array.from((_s = this.moduleManager.events.get("messageEdit")) !== null && _s !== void 0 ? _s : []);
            while (i < events2.length) {
              const [file, handler] = events2[i];
              try {
                if (yield handler(...args)) {break;}
                i++;
              } catch (error) {
  ...
  constructor(config, options = {}) {
    ...
    this.getChannel = (id, type, stringType) => {
      ...
      if (channel.type !== type) {return this.wrongTypeErr(id, stringType, discord_js_1.default.ChannelType[channel.type]);}
      return channel;
    };
  ...
  wrongTypeErr(id, strType, expected) {
    ...
    if ((_a = this.config.strictTypes) === null || _a === void 0 ? void 0 : _a.channels) {throw new Error(`Expected a ${expected} channel but got a ${strType} instead. (id: ${id})`);} else {return null;}
  }
  getTextChannel(id) {
